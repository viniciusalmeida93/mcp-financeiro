...
export default function ProximosVencimentosCard({ proximasContas, loading }) {
          ...
          {empresa.length > 0 && (
            <div style={{ marginBottom: 'var(--spacing-sm)' }}>
              ...
              {empresa.map(conta => {
                ...
                return (
                  <div key={conta.id} className={`list-item list-item--empresa`}>
                    ...
                    <div className={`list-item__value ${atrasado ? 'list-item__value--negative' : ''}`}>
          ...
          {pessoal.length > 0 && (
            <div>
              ...
              {pessoal.map(conta => {
                ...
                const atrasado = venc < hoje
                return (
                  <div key={conta.id} className={`list-item list-item--pessoal`}>
                    <div className="list-item__body">
                      <div className="list-item__title">{conta.nome}</div>
                      <div className="list-item__subtitle">{formatDateShort(conta.proximoVencimento)}</div>
                    </div>
